...
def evaluate(program_path: str, *, repo_root: Path | None = None):
    ...
    benchmark_dir = (
        repo_root / "benchmarks" / "Robotics" / "PIDTuning"
    ).resolve()
    if not benchmark_dir.is_dir():
        benchmark_dir = (repo_root / "Robotics" / "PIDTuning").resolve()
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="fe_pid_tuning_")).resolve()
    try:
        sandbox_program = work_dir / "init.py"
        ...
        spec = importlib.util.spec_from_file_location("fe_pid_tuning_eval", eval_path)
        ...
        if feasible:
            ...
            artifacts["error_message"] = "infeasible PID gains"
